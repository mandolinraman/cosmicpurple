...
multi_red.reduce(metrics, b0)
b0 = [np.max(metrics[locations[i]]) for i in range(num_buckets)]
b0 = [np.max(metrics[j] for j in locations[i]) for i in range(num_buckets)]
b0 = [np.max([metrics[j] for j in locations[i]]) for i in range(num_buckets)]

# soft reduce
b1 = np.zeros(num_buckets)
multi_red.soft_reduce(metrics, b1)
b1 = [logsumexp(metrics[locations[i]]) for i in range(num_buckets)]
